...
async function createAccountAndLogin(username: string, password: string): Promise<string> {
	const createRes = await call("/admin/accounts", jsonBody({ username, password }));
	expect(createRes.status).toBe(201);
	const loginRes = await call("/login", jsonBody({ username, password }));
