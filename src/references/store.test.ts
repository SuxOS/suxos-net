...
describe("createReference", () => {
	it("stamps dateAdded server-side from the provided clock", async () => {
		...
		const result = await createReference(kv, rateLimiter, FICTIONAL_INPUT, now);
	...
	it("rejects a duplicate id", async () => {
		const kv = createMemoryKv();
		const rateLimiter = createRateLimiterNamespace(kv);
		await createReference(kv, rateLimiter, FICTIONAL_INPUT);
		const dupe = await createReference(kv, rateLimiter, FICTIONAL_INPUT);
	...
	it("omits sourceUrl entirely when not provided", async () => {
		const kv = createMemoryKv();
		const rateLimiter = createRateLimiterNamespace(kv);
		const result = await createReference(kv, rateLimiter, FICTIONAL_INPUT);
...
describe("getReference / listReferences", () => {
	...
	it("lists every curated reference", async () => {
		...
		const rateLimiter = createRateLimiterNamespace(kv);
		await createReference(kv, rateLimiter, FICTIONAL_INPUT);
		await createReference(kv, rateLimiter, { ...FICTIONAL_INPUT, id: "ref-demo-2" });
...
describe("updateReference", () => {
	it("edits a field and leaves dateAdded and other fields untouched", async () => {
		...
		const rateLimiter = createRateLimiterNamespace(kv);
		const now = new Date("2026-07-18T00:00:00.000Z");
		await createReference(kv, rateLimiter, FICTIONAL_INPUT, now);

		const result = await updateReference(kv, rateLimiter, FICTIONAL_INPUT.id, { text: "Updated fictional text." });
	...
	it("returns not-found for a reference that does not exist", async () => {
		const kv = createMemoryKv();
		const rateLimiter = createRateLimiterNamespace(kv);
		const result = await updateReference(kv, rateLimiter, "no-such-id", { text: "x" });
